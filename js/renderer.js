...
function buildBars(array) {
  ...
  // Show labels always for small arrays
  if (array.length <= 10) {
...
const stepTypeToStateClass = (type) => {
  const map = { swap: 'state-swapping', compare: 'state-comparing', sorted: 'state-sorted', pivot_set: 'state-pivot' };
...
function renderCallStack(stepIndex) {
  ...
  DOM.callstackFrames.innerHTML = visible
    .map((frame, i) => {
      ...
      return `<div class="callstack-frame ${depthClass}">
        quickSort(${frame.low}, ${frame.high}) <span style="opacity:0.5">depth=${frame.depth}</span>
      </div>`;
    })
    .join('');
}
